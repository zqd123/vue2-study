...
Vue.component("tabs", {
    ...
    <div class="tabs">
        ...
        <div class="tabs-bar">
            ...
            <div :class="tabCls(item)" v-for="(item,index) in navList" @click="handleChange(index)" >
                {{item.label}}
                <button v-if="isClose" @click="closeTab(item.name)">x</button>
            </div>
        </div>
  ...
  methods: {
    ...
    //   const tabs = this.getTabs();
      this.navList = this.navList.filter((tab) => tab.name !== name);
      this.$emit('edit', name)
    //   this.$children = this.$children.filter((tab) => tab.name !== name);
    },
  },
  watch: {
    value(val) {
